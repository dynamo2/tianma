...
@ManagedBean(name = "orderBakBean")
@ViewScoped
@ViewRetained
@WindowDisposed
public class OrderBakBean extends AbstractManagementBean implements Serializable {
    ...
    public void init() {
        ...
        try {
            printTemplates
                    .load(this.getClass().getClassLoader().getResourceAsStream("/form_print_template.properties"));
        } catch (IOException e) {
    ...
    public List<FormMetadata> getPublishedFormMetadataList() {
        if (null == publishedFormMetadataList) {
            ...
            for (FormMetadata form : forms) {
                if (FormMetadataStatus_ENUM.PUBLISHED.name().equalsIgnoreCase(form.getFormStatus())) {
    ...
    public List<FormMetadata> getPublishedCreatableFormMetadataList() {
        ...
        for (FormMetadata formMetadata : forms) {
            if (formMetadata.getCanCreateAccounts().contains(appSessionParams.getCurrentAccount().getAccount())) {
    ...
    public void setPublishedFormMetadataList(List<FormMetadata> publishedFormMetadataList) {
    ...
    private void fillinData(DynamicFormDefault order) {
        ...
        if (order.getParentFormBizId() != null) {
            parentForm = formMetadataService.loadDynamicFormDefaultDataByBizKey(order.getParentFormBizId());
        } else {
            Long parentFormId = getParentFormRecordId();
            if (parentFormId != null && parentFormId > 0L) {
                parentForm = formMetadataService.loadDynamicFormDefaultDataById(parentFormId);
            }
        }
    ...
    public void saveOrder() {
        formMetadataService.addOrUpdateFormDefault(formMetadata, columnValue, order);
        ...
        if (parentFormRecordId != null && parentFormFieldColumnName != null) {
            formMetadataService.updateParentFormRecord(parentFormRecordId, parentFormFieldColumnName, order);
        }
        ...
        if (order == null) {
            if (orderId == null && formBizKey == null) {
                ...
                if (appSessionParams.isRoleCustomer()) {
                    order.setCustomerId(appSessionParams.getCurrentAccount().getCustomerId());
                }
                order.setCustomerId(customerId);
                order.setFormMetadataId(formMetadataId);
            } else if (formBizKey != null) {
                order = formMetadataService.loadDynamicFormDefaultDataByBizKey(formBizKey);
    ...
    public boolean isToMeOrder() {
        return appSessionParams.getCurrentAccount().getAccount().equals(order.getAssigneeAccount());
    }
    ...
    public void listFormData() {
        if (formMetadataId == 0) {
            ...
            if (customerId != null && customerId != 0) {
                formDataList = formMetadataService.listFormDefaultForCustomer(formMetadataId, customerId);
            } else {
                customerId = null;
                formDataList = formMetadataService.listFormDefault(formMetadataId);
            }
    ...
    public void showSearchCriteria() {
        ...
        for (FormFieldMetadata f : fields) {
            FormFieldTypes_ENUM t = FormFieldTypes_ENUM.valueOf(f.getFieldType().toUpperCase());
            switch (t) {
                case VARCHAR:
    ...
    public void search() {
        formDataList = formMetadataService.searchByCriteria(searchCriteriaValueMap,null, formMetadataId);
    }
    ...
    public List<FormMetadata> getPublishedCustomerProfileFormMetadataList() {
        if (publishedCustomerProfileFormMetadataList == null) {
            publishedCustomerProfileFormMetadataList = formMetadataService.listAllFormMetadata();
        }
    ...
    public List<FormWorkflow> getFormWorkflowSteps() {
        if (formWorkflowSteps == null && getOrder() != null) {
            formWorkflowSteps = formMetadataService.listWorkFlowSteps(formMetadataId);
            if (formWorkflowSteps != null && !formWorkflowSteps.isEmpty()) {
                ...
                for (FormWorkflow step : formWorkflowSteps) {
                    workflowStepsMappedByStatusName.put(step.getStatusName(), step);
                    workflowStepsMappedByStepName.put(step.getStepName(), step);
                    workflowStepIndexMappedByStepName.put(step.getStepName(), index++);
                }
            }
        }
    ...
    public boolean isWorkFlowApprovable() {
        if (order == null || !appSessionParams.getCurrentAccount().getAccount().equals(order.getAssigneeAccount())) {
            return false;
        }
    ...
    public FormWorkflow getWorkFlowApproveToStep() {
        ...
        if (step == null) {
            ...
            if (nextStepName != null && !nextStepName.isEmpty()) {
                ...
                Integer currentStepIndex = workflowStepIndexMappedByStepName.get(step.getStepName());
    ...
    public FormWorkflow getWorkFlowBackToStep() {
        ...
        FormWorkflow step = workflowStepsMappedByStatusName.get(currentStatus);
        if (step != null) {
            Integer index = workflowStepIndexMappedByStepName.get(step.getStepName());
            if (index > 0) {
    ...
    private void changeAssigneeAndStatus(DynamicFormDefault o, FormWorkflow nextStep) {
        if (nextStep == null) {
            return;
        }
    ...
    public boolean isFroze() {
        ...
        return FormRecordStatus_ENUM.END.name().equalsIgnoreCase(order.getStatus())
                || FormRecordStatus_ENUM.INVALID.name().equalsIgnoreCase(order.getStatus());
    }
    ...
    public void setFormBizKey(String formBizKey) {
        try {
            this.formBizKey = new String(formBizKey.getBytes("ISO-8859-1"), "UTF-8");
    ...
    public boolean isReadable() {
        getOrder();
        return getFormMetadata().getCanReadAccountList().contains(appSessionParams.getCurrentAccount().getAccount());
    }
            ...
            if (getOrder() != null && getOrder().getFormName() != null) {
                if (printTemplates.containsKey(getOrder().getFormName())) {
                    printReviewPage = "../dynamic_form/report_template/"
                            + printTemplates.getProperty(getOrder().getFormName()) + "?formMetadataId="
                            + formMetadataId + "&formRecordId=" + orderId;
                } else {
                    printReviewPage = "../dynamic_form/form_print.jsf?formMetadataId=" + formMetadataId
                            + "&formRecordId=" + orderId;
                }
            } else {
                return "#";
            }
    ...
    public List<FileUpload> getAttachedFiles() {
        if (attachedFiles == null && order != null && order.getId() != null) {
            attachedFiles = fileuploadService.findByFields(new String[] { "forId", "forType" },
                    new Object[] { order.getId(), FILE_FOR_ENUM.FORM.name() });
        }
