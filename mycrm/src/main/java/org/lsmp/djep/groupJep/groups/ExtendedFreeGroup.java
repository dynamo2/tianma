 ...
 * Positive integer powers are allowed and division by constants. TODO implement
 * polynomial division and remainder (mod).
 * 
 * @author Rich Morris Created on 07-Dec-2004
 */
public class ExtendedFreeGroup extends FreeGroup implements HasPowerI, HasDivI {
	...
	/**
	 * Limited implementation of power, only works with integer powers. Second
	 * argument should be an Integer.
	 */
	public Number pow(Number a, Number b) {
		FreeGroupElement exp = (FreeGroupElement) b;
		if (!isConstantPoly(exp))
			throw new IllegalArgumentException("Powers only supported for integer exponant. Current exponant is "
					+ exp.toString());
	...
	/**
	 * Division of Polynomials, discards remainder. Not yet implemented.
	 */
