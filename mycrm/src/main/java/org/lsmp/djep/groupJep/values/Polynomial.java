 ...
 * 
 * @author Rich Morris Created on 09-Mar-2004
 */
public class Polynomial extends Number {
	...
	/**
	 * Construct a polynomial over a ring. In general the valueOf method should
	 * be used to construct a new polynomial.
	 * 
	 * @param baseRing
	 *            the underlying ring of the polynomial.
	 * @param symbol
	 *            the symbol used to display the polynomial
	 * @param coeffs
	 *            an array of coeficients in the base ring coeff[0] is constant,
	 *            coeff[1] is coefficient of t etc.
	 */
	...
	/**
	 * Sub classes can change the coefficients. Other methods should treat
	 * polynomials as imutable.
	 */
	...
	/**
	 * Factory method to create a polynomial with the given coefficients. Sub
	 * classes should overwrite this method to costruct objects of the correct
	 * type.
	 */
	...
	public Polynomial pow(int exp) {
		if (exp == 0)
			return valueOf(new Number[] { baseRing.getONE() });
		if (exp == 1)
			return valueOf(this.getCoeffs());
		if (exp < 0)
	...
	public String toString() {
		...
		for (int i = degree; i >= 0; --i) {
			...
			if (s.equals("0") || this.baseRing.equals(coeffs[i], baseRing.getZERO()))
				continue;

			// apart from first add a + sign if positive
			if (i != degree && !s.startsWith("-"))
				sb.append("+");
	...
	public int intValue() {
		return coeffs[0].intValue();
	}

	/** value of constant coeff. */
	public long longValue() {
		return coeffs[0].longValue();
	}

	/** value of constant coeff. */
	public float floatValue() {
		return coeffs[0].floatValue();
	}
	...
	/**
	 * returns the complex value of this polynomial. Where the value of the
	 * symbol is replaced by rootVal.
	 */
