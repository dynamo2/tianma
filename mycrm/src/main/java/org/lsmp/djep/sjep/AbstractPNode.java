 ...
 * 
 * @author Rich Morris Created on 22-Dec-2004
 */
public abstract class AbstractPNode implements PNodeI {
	...
	public PNodeI mul(PNodeI node) throws ParseException {
		...
		if (this.compareTo(node) < 0)
			return new Monomial(pc, pc.oneConstant, new PNodeI[] { this, node }, new PNodeI[] { pc.oneConstant,
					pc.oneConstant });

		return new Monomial(pc, pc.oneConstant, new PNodeI[] { node, this }, new PNodeI[] { pc.oneConstant,
				pc.oneConstant });
	}
	...
	public int compareTo(PNodeI node) {
		...
		if (this instanceof Monomial || this instanceof Polynomial)
			throw new IllegalStateException("Comparison failed " + this.getClass().getName() + " "
		...
		throw new IllegalArgumentException("Comparison failed " + this.getClass().getName() + " "
				+ node.getClass().getName());
	}
