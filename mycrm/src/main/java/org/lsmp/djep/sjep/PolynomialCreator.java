 ...
 * 
 * <p>
 * A total ordering of all expressions is used. As the representation is
 * constructed the total ordering of terms is maintained. This helps ensure that
 * polynomials are always in their simplest form and also allows comparison of
 * equations.
 * </p>
 * <p>
 * The following sequence illustrates current ordering. This ordering may change
 * without warning.
 * <ul>
...
public class PolynomialCreator extends DoNothingVisitor {
	 ...
	 *             if expression cannot be converted.
	 */
	...
	/**
	 * Expands an expression. Will always expand brackets for multiplication and
	 * simple powers. For instance <code>(1+x)^3 -> 1+3x+3x^2+x^3</code>
	 * 
	 * @param node
	 *            top node to expression to be simplified.
	 * @return a simplified expression
	...
	/**
	 * Compares two nodes. Uses a total ordering of expressions. Expands
	 * equations before comparison.
	 * 
	 * @param node1
	 * @param node2
	...
	/**
	 * Compares two nodes. Uses a total ordering of expressions. May give some
	 * false negatives is simplification cannot reduce two equal expressions to
	 * the same canonical form. Expands equations before comparison.
	 * 
	 * @param node1
	 * @param node2
	...
	public Object visit(ASTFunNode node, Object data) throws ParseException {
		...
		for (int i = 0; i < args.length; ++i)
			if (!(args[i] instanceof PConstant)) {
				allConst = false;
				break;
			}
		...
		// throw new
		// ParseException("Polynomial: Sorry don't know how to convert "+node.getName());
	}
	...
	Object raise(Object a, Object b) throws ParseException {
		return ((Power) os.getPower().getPFMC()).power(a, b);
	}
	...
	int cmp(Object a, Object b) throws ParseException {
		...
		throw new IllegalArgumentException("Sorry don't know how to compare " + a + " (" + a.getClass().getName()
				+ ") and " + b + " (" + b.getClass().getName() + ")");
	}
}
