...
public class Monomial extends AbstractPNode {
	...
	Monomial(PolynomialCreator pc, PConstant coeff, PNodeI vars[], PNodeI powers[]) {
		...
		if (vars.length != powers.length)
			throw new IllegalArgumentException(
					"Monomial.valueOf length of variables and powers must be equal. they are " + vars.length + " "
							+ powers.length);
		this.coeff = coeff;
		this.vars = vars;
		this.powers = powers;
	...
	public PNodeI add(PNodeI node) throws ParseException {
		if (node instanceof PVariable) {
			if (this.equalsIgnoreConstant(node)) {
				return valueOf((PConstant) coeff.add(pc.oneConstant), vars, powers);
			}
		}
		if (node instanceof Monomial) {
			Monomial mon = (Monomial) node;
			if (this.equalsIgnoreConstant(mon)) {
				return valueOf((PConstant) coeff.add(mon.coeff), vars, powers);
			}
		}
	...
	public PNodeI sub(PNodeI node) throws ParseException {
		if (node instanceof PVariable) {
			if (this.equalsIgnoreConstant(node)) {
				return valueOf((PConstant) coeff.sub(pc.oneConstant), vars, powers);
			}
		}
		if (node instanceof Monomial) {
			Monomial mon = (Monomial) node;
			if (this.equalsIgnoreConstant(mon)) {
				return valueOf((PConstant) coeff.sub(mon.coeff), vars, powers);
			}
		}
	...
	public boolean equals(PNodeI node) {
		if (!(node instanceof Monomial))
			return false;
		if (!coeff.equals(((Monomial) node).coeff))
			return false;
		return equalsIgnoreConstant((Monomial) node);
	}

	boolean equalsIgnoreConstant(Monomial mon) {
		if (vars.length != mon.vars.length)
			return false;
		for (int i = 0; i < vars.length; ++i) {
			if (!vars[i].equals(mon.vars[i]))
				return false;
			if (!powers[i].equals(mon.powers[i]))
				return false;
		}
		return true;
	}
	...
	/**
	 * Compare this to argument. x < y 2 x < 3 x x < x^2 x^2 < x^3 x < x y TODO
	 * x y < x^2
	 * 
	...
	public Node toNode() throws ParseException {
		...
		for (int i = 0; i < vars.length; ++i) {
			...
			if (powers[i].isOne())
				...
				args[pos++] = pc.nf.buildOperatorNode(pc.os.getPower(), vars[i].toNode(), powers[i].toNode());
		}
		Node top;
		if (args.length == 0)
			top = coeff.toNode();
			...
			top = pc.nf.buildOperatorNode(pc.os.getMultiply(), args);
		...
		for (int i = 0; i < vars.length; ++i) {
			if (negativePower(powers[i])) {
				...
				if (powers[i] instanceof PConstant && ((PConstant) powers[i]).isMinusOne())
					...
					divisors[pos++] = pc.nf.buildOperatorNode(pc.os.getPower(), vars[i].toNode(), pow.toNode());
			}
		}
		Node bottom;
		if (divisors.length == 1)
			...
			bottom = pc.nf.buildOperatorNode(pc.os.getMultiply(), divisors);
