...
/**
 * Differentiates a division with respect to var. diff(y/z,x) ->
 * (diff(y,x)*z-y*diff(z,x))/(z*z)
 */
		...
		if (nchild == 2)
			return nf.buildOperatorNode(
					...
					nf.buildOperatorNode(op.getSubtract(),
							nf.buildOperatorNode(op.getMultiply(), dchildren[0], djep.deepCopy(children[1])),
							nf.buildOperatorNode(op.getMultiply(), djep.deepCopy(children[0]), dchildren[1])),
					nf.buildOperatorNode(op.getMultiply(), djep.deepCopy(children[1]), djep.deepCopy(children[1])));
