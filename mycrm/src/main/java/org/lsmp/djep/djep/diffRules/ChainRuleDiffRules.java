...
public abstract class ChainRuleDiffRules implements DiffRulesI {
	...
	public Node differentiate(ASTFunNode node, String var, Node[] children, Node[] dchildren, DJep djep)
		...
		if (nRules != children.length)
			throw new ParseException("Error differentiating " + name + " number of rules " + nRules
					+ " != number of arguments " + children.length);
	...
	/**
	 * Changes the names of variables in the rules to make the chain rule
	 * easier. Substitutes the variable x,y with @1,@2 and x1,x2,x3,... with
	 * @1,@2,@3,...
	 * 
	 * @throws ParseException
	 */
	/*
	 * protected void fixVarNames() throws ParseException { NameChangeVisitor nc
	 * = new NameChangeVisitor(dv.djep); int nrules = this.getNumRules();
	 * if(nrules==0) return; else if(nrules == 1) rules[0] =
	 * nc.changeNames(rules[0],new String[]{"x"},new String[]{"@1"}); else
	 * if(nrules == 2) { rules[0] = nc.changeNames(rules[0],new String
	 * []{"x","y"},new String []{"@1","@2"}); rules[1] =
	 * nc.changeNames(rules[1],new String []{"x","y"},new String []{"@1","@2"});
	 * } else { String[] oldvars = new String[nrules]; String[] newvars = new
	 * String[nrules]; for(int i=0;i<nrules;++i) { oldvars[i] = "x" +
	 * String.valueOf(i); newvars[i] = "@" + String.valueOf(i); } rules[0] =
	 * nc.changeNames(rules[0],oldvars,newvars); } }
	 */
