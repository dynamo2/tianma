...
/**
 * Diffrentiates a power with respect to var. If m is a a number diff(y^m,x) ->
 * m * y^(m-1) otherwise diff(y^z,x) -> z * y^(z-1) * diff(y,x) + y^z * ln(z) *
 * diff(z,x)
 */
	...
	public Node differentiate(ASTFunNode node, String var, Node[] children, Node[] dchildren, DJep djep)
		...
		if (tu.isConstant(children[1])) {
			...
			// Node e = opSet.buildMultiplyNode(a,d);
				...
				return nf.buildOperatorNode(op.getMultiply(), djep.deepCopy(children[1]),
						nf.buildOperatorNode(
								op.getMultiply(),
								nf.buildOperatorNode(op.getPower(), djep.deepCopy(children[0]),
			...
			return nf.buildOperatorNode(
					...
					nf.buildOperatorNode(
							...
							nf.buildOperatorNode(
									...
									nf.buildOperatorNode(op.getSubtract(), djep.deepCopy(children[1]),
											nf.buildConstantNode(tu.getONE()))), dchildren[0]));

		}
		// z * y^(z-1) * diff(y,x) + y^z * ln(z) * diff(z,x)
		{
			return nf.buildOperatorNode(op.getAdd(), nf.buildOperatorNode(op.getMultiply(), // z
																							// *
																							// y^(z-1)
																							// *
																							// diff(y,x)
					nf.buildOperatorNode(op.getMultiply(), // z * y^(z-1)
							...
							nf.buildOperatorNode(op.getPower(), // y^(z-1)
									...
									nf.buildOperatorNode(op.getSubtract(), // z-1
											djep.deepCopy(children[1]), // z
											djep.getNodeFactory().buildConstantNode(tu.getONE())))), dchildren[0]), // diff(y,x)
					nf.buildOperatorNode(op.getMultiply(), // + y^z * ln(z) *
															// diff(z,x)
							nf.buildOperatorNode(op.getMultiply(), nf.buildOperatorNode(op.getPower(), // y^z
									djep.deepCopy(children[0]), djep.deepCopy(children[1])), djep.getNodeFactory()
									.buildFunctionNode("ln", funTab.get("ln"), // ln(z)
											new Node[] { djep.deepCopy(children[0]) })), dchildren[1]));
