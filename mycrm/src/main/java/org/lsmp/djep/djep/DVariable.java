 ...
 * {@link PartialDerivative PartialDerivative}. The derivatives are stored in a
 * hashtable index by the sorted names of derivatives. i.e. d^2f/dxdy, and
 * d^2f/dydx will both be indexed by {"x","y"}. df/dx is indexed by {"x"},
 * d^2f/dx^2 is index by {"x","x"}. Partial derivatives are calculated as
 * required by the findDerivative method.
 * 
...
public class DVariable extends XVariable {
	...
	/**
	 * Produces a string to represent the derivative. The string will be of the
	 * form "dx^2/dxdy". This string is used to index the derivatives of a
	 * variable.
	 * 
	 * @param rootname
	 *            name of the variable we are calculating the derivative of.
	 * @param dnames
	 *            An array of the names of each of the partial derivatives.
	...
	/**
	 * Sets the derivative wrt the variables specified in deriv names. Note the
	 * names are sorted so that d^2z/dxdy = d^2z/dydx.
	 */
	...
	/**
	 * Finds the derivative of this variable wrt the names. If the derivative
	 * already exists just return that. Otherwise use the visitor to calculate
	 * the derivative. If the derivative cannot be calculated (say if the
	 * equation is null) then return null.
	 * 
	 * @param derivnames
