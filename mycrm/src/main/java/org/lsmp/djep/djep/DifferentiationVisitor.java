 ...
 * {@link org.nfunk.jep.ParserVisitor ParserVisitor}. The rules for
 * differentiating specific functions are contained in object which implement
 * {@link DiffRulesI DiffRulesI} A number of inner classes which use this
 * interface are defined for specific function types. In particular
 * {@link MacroDiffRules MacroDiffRules} allow the rule for differentiation to
 * be specified by strings. New rules can be added using
 * {@link DJep#addDiffRule} method.
 * 
 * @author R Morris Created on 19-Jun-2003
 */
public class DifferentiationVisitor extends DeepCopyVisitor {
	...
	void addDiffRule(DiffRulesI rule) {
		...
		if (DEBUG)
			System.out.println("Adding rule for " + rule.getName());
	}
	...
	/**
	 * Prints all the differentation rules for all functions on specified
	 * stream.
	 */
	 ...
	 *             if some error occurred while trying to differentiate, for
	 *             instance of no rule supplied for given function.
	...
	/**
	 * Applies differentiation to a function. Used the rules specified by
	 * objects of type {@link DiffRulesI}.
	 * 
	 * @param node
	 *            The node of the function.
	 * @param data
	 *            The variable to differentiate wrt.
	 **/
	...
	/**
	 * Differentiates a variable. May want to alter behaviour when using multi
	 * equation as diff(f,x) might not be zero.
	 * 
