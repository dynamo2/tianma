...
/**
 * A fast evaluation algorithm for equations using Vectors and Matrix over the
 * Doubles. This is based around reverse polish notation (hence the name M Rp
 * Eval) and is optimised for speed at every opportunity.
 * <p>
 * To use do
 * 
 * <pre>
 ...
 * 
 * <p>
 * The real use of this class is when an equation (or set of equations) need to
 * be repeatedly evaluated with different values for the variables. MRpEval use
 * an internal store for variable values different from those used in the main
 * Jep classes. Changes in the Jep variable values, say by calling
 * {@link org.nfunk.jep.JEP#setVarValue JEP.setVarValue}, are reflected in
 * changes in MRpEval variables, (the converse does not hold). A more efficient
 * way is to use <code>int ref=getVarRef(var)</code> to return an index number
 * of the variable and then calling <code>setVarVal(ref,value)</code> to set its
 * value. For example
 * 
 * <pre>
 * MRpCommandList list = rpe.compile(node);
 ...
 * 
 * <p>
 * Combining mrpe with differentation requires special techniques to cope with
 * that fact that internal equations are used
 * <p>
 * The compile methods converts the expression represented by node into a string
 * of commands. For example the expression "1+2*3" will be converted into the
 ...
 * </pre>
 * 
 * The evaluate method executes these methods sequentially using a stack
 * (actually a set of stacks) and returns the last object on the stack.
 * <p>
 * A few cautionary notes: the values returned by evaluate are references to
 * internal variables, their values will change at the next call to compile or
 * evaluate. Its very unlikely to be thread safe. It only works over doubles;
 * expressions with complex numbers or strings will cause problems. It is tuned
 * to work best for expressions involving scalers and 2, 3 and 4 dimensional
 * vectors and matricies, larger vectors and matrices will be noticeably slower.
 * The cleanUp function should be used when you no longer need the evaluator,
 * this stops the evaluator listening to Variable through the java.util.Observer
 * interface.
 * <p>
 * <b>Implementation notes</b> A lot of things have been done to make it as fast
 * as possible:
 * <ul>
 ...
 * <li>Vectors and Matrices are instances of VecObj and MatObj optimised for
 * speed. For instance a 2 by 2 matrix is an instance of Mat22Obj whose elements
 * are represented by the fields a,b,c,d. This eliminates bound checking on
 * arrays.</li>
 * <li>Each possible vector and matrix operation has been hand coded, and there
 * are a lot of methods (27 just for matrix multiplication!).</li>
 * <li>The values of variables are kept on local arrays for fast access. These
 * values are kept in sync with the main Jep Variables by using the
 * java.util.Observer interface.</li>
 * </ul>
 * 
 * <p>
 * For each type of vector or matrix (i.e. 2D vecs, 3D vecs, 4D vecs, 2 by 2
 * matrices ... 4 by 4 matrices. there is a corresponding class V2Obj, M22Obj
 * etc. which stores the values and another class V2Store, M22Store etc. Each
 * Store class contains a stack, a heap and a array of variable values. During
 * evaluation objects are pushed and popped from the stack when a new object is
 * needed it is taken from the heap. The operation is illustrated by the add
 * method for 2 by 2 matrices.
 * 
 * <pre>
 ...
 * </pre>
 * 
 * @author Rich Morris Created on 14-Apr-2004
 */
public final class MRpEval implements ParserVisitor {
	...
	/**
	 * Compile the expressions to produce a set of commands in reverse Polish
	 * notation.
	 */
	 ...
	 * 
	 * @param ref
	 *            the reference number for the variable (found using
	 *            {@link #getVarRef(org.lsmp.djep.matrixJep.MatrixVariableI)})
	 * @param val
	 * @throws ParseException
	 */
	...
	private final static class ScalerObj extends MRpRes {
		...
		public final void copyToVecMat(MatrixValueI res) throws ParseException {
			if (!res.getDim().is0D())
				throw new ParseException("CopyToVecMat: dimension of argument " + res.getDim()
						+ " is not equal to dimension of object " + getDims());
	...
	/**
	 * Base class for storage for each type of data. Each subclass should define
	 * 
	 * <pre>
	 * private Obj stack[];
	 * private Obj heap[];
	 ...
	 * </pre>
	 * 
	 * where Obj is an Object of the specific type, eg V2Obj. Memory for the
	 * data is allocated from the heap and the stack is the current data used
	 * for calculations. Data for Variables is stored in vars and references to
	 * the Variables in varRefs.
	 */
	private abstract static class ObjStore implements Observer {
		...
		/**
		 * Callback function for Observable Variables. Called whenever the value
		 * of a variable is changed so the private list of variables is kept in
		 * sync.
		 */
	...
	private final class M24Store extends MatStore {
		...
		final void eq() {
			M24Obj r = stack[--sp];
			M24Obj l = stack[--sp];
			if (l.a == r.a && l.b == r.b && l.c == r.c && l.d == r.d && l.e == r.e && l.f == r.f && l.g == r.g
					&& l.h == r.h)
				scalerStore.stack[scalerStore.sp++] = 1.0;
			else
				scalerStore.stack[scalerStore.sp++] = 0.0;
	...
	private static final class M34Obj extends MatObj {
		...
		public double[][] toArrayMat() {
			return new double[][] { { a, b, c, d }, { e, f, g, h }, { i, j, k, l } };
		}
	}
	...
	private final class M34Store extends MatStore {
		...
		final void eq() {
			...
			M34Obj l = stack[--sp];
			if (l.a == r.a && l.b == r.b && l.c == r.c && l.d == r.d && l.e == r.e && l.f == r.f && l.g == r.g
					&& l.h == r.h && l.i == r.i && l.j == r.j && l.k == r.k && l.l == r.l)
				scalerStore.stack[scalerStore.sp++] = 1.0;
			else
				scalerStore.stack[scalerStore.sp++] = 0.0;
		}
	...
	private final class M44Store extends MatStore {
		...
		final void alloc() {
			...
			System.arraycopy(vars, 0, newvars, 0, vars.length);
			for (int i = vars.length; i < varRefs.size(); ++i)
				newvars[i] = new M44Obj();
			vars = newvars;
		}
		...
		final void eq() {
			...
			M44Obj l = stack[--sp];
			if (l.a == r.a && l.b == r.b && l.c == r.c && l.d == r.d && l.e == r.e && l.f == r.f && l.g == r.g
					&& l.h == r.h && l.i == r.i && l.j == r.j && l.k == r.k && l.l == r.l && l.m == r.m && l.n == r.n
					&& l.o == r.o && l.p == r.p)
				scalerStore.stack[scalerStore.sp++] = 1.0;
			else
				scalerStore.stack[scalerStore.sp++] = 0.0;
	...
	public final Object visit(ASTFunNode node, Object data) throws ParseException {
		...
		if (mnode.isOperator()) {
			...
			if (op == opSet.getAdd()) {
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(EQ, getDimType(ldims));
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(NE, getDimType(ldims));
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(LT, SCALER);
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(GT, SCALER);
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(LE, SCALER);
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(GE, SCALER);
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(AND, SCALER);
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(OR, SCALER);
				return null;
				...
				scalerStore.incStack();
				decByDim(rdims);
				curCommandList.addCommand(NOT, SCALER);
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(MOD, SCALER);
				return null;
				...
				decByDim(ldims);
				decByDim(rdims);
				curCommandList.addCommand(POW, SCALER);
				return null;
			}
	...
	private final void dotV2() {
		...
		scalerStore.stack[scalerStore.sp++] = l.a * r.a + l.b * r.b;
	}

	private final void crossV2() {
		V2Obj r = v2Store.stack[--v2Store.sp];
		V2Obj l = v2Store.stack[--v2Store.sp];
		scalerStore.stack[scalerStore.sp++] = l.a * r.b - l.b * r.a;
	}

	private final void dotV3() {
		V3Obj r = v3Store.stack[--v3Store.sp];
		V3Obj l = v3Store.stack[--v3Store.sp];
		scalerStore.stack[scalerStore.sp++] = l.a * r.a + l.b * r.b + l.c * r.c;
	}
	...
	private final void dotV4() {
		...
		scalerStore.stack[scalerStore.sp++] = l.a * r.a + l.b * r.b + l.c * r.c + l.d * r.d;
	}
	...
	private final void unitaryFunction(short fun) {
		...
		case ASINH:
			r = Math.log(r + Math.sqrt(1 + r * r));
			break;
		case ACOSH:
			r = Math.log(r + Math.sqrt(r * r - 1));
			break;
		case ATANH:
			r = Math.log((1 + r) / (1 - r)) / 2.0;
			break;
