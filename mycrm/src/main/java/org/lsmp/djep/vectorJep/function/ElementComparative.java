...
/**
 * Multiplies any number of Vectors or Matrices element by element. TODO could
 * be much more efficient when called through run.
 * 
...
public class ElementComparative extends Comparative implements BinaryOperatorI {
	...
	public MatrixValueI calcValue(MatrixValueI res, MatrixValueI lhs, MatrixValueI rhs) throws ParseException {
		...
		for (int i = 0; i < len; ++i) {
			...
			switch (id) {
			case LT:
				val = lt(lhs.getEle(i), rhs.getEle(i));
				break;
			case GT:
				val = gt(lhs.getEle(i), rhs.getEle(i));
				break;
			case LE:
				val = le(lhs.getEle(i), rhs.getEle(i));
				break;
			case GE:
				val = ge(lhs.getEle(i), rhs.getEle(i));
				break;
			case NE:
				val = ne(lhs.getEle(i), rhs.getEle(i));
				break;
			case EQ:
				val = eq(lhs.getEle(i), rhs.getEle(i));
				break;
			}
