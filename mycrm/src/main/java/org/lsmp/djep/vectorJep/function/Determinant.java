...
public class Determinant extends PostfixMathCommand implements UnaryOperatorI {
	...
	public MatrixValueI calcValue(MatrixValueI res, MatrixValueI lhs) throws ParseException {
		...
		if (mat.getNumRows() == 2) {
			res.setEle(0,
					sub.sub(mul.mul(mat.getEle(0, 0), mat.getEle(1, 1)), mul.mul(mat.getEle(1, 0), mat.getEle(0, 1))));
		} else if (mat.getNumRows() == 3) {
			// | a b c |
			// | d e f | -> a e j + b f g + c d h - a f h - b d i - c e g
			// | g h i |
			Object r1 = mul.mul(mat.getEle(0, 0), mul.mul(mat.getEle(1, 1), mat.getEle(2, 2)));
			Object r2 = mul.mul(mat.getEle(0, 1), mul.mul(mat.getEle(1, 2), mat.getEle(2, 0)));
			Object r3 = mul.mul(mat.getEle(0, 2), mul.mul(mat.getEle(1, 0), mat.getEle(2, 1)));
			Object r4 = mul.mul(mat.getEle(0, 0), mul.mul(mat.getEle(1, 2), mat.getEle(2, 1)));
			Object r5 = mul.mul(mat.getEle(0, 1), mul.mul(mat.getEle(1, 0), mat.getEle(2, 2)));
			Object r6 = mul.mul(mat.getEle(0, 2), mul.mul(mat.getEle(1, 1), mat.getEle(2, 0)));
		...
		// throw new
		// ParseException("Sorry can only calculate determinants for 2 by 2 and 3 by 3 matricies");
		return res;
	}
	...
	/**
	 * Calculates the determinant of an array Uses the fact that | a b c | | d e
	 * f | = a | e f | - b | d f | + c | d e | | g h i | | h i | | g i | | g i |
	 */
	public Object det(Object[][] mat) throws ParseException {
		if (mat.length == 1)
			return mat[0][0];
		if (mat.length == 2) {
			return sub.sub(mul.mul(mat[0][0], mat[1][1]), mul.mul(mat[1][0], mat[0][1]));
		}
