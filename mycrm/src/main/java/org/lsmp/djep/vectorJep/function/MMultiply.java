...
public class MMultiply extends Multiply implements BinaryOperatorI {
	...
	public MatrixValueI calcValue(MatrixValueI res, MatrixValueI param1, MatrixValueI param2) throws ParseException {
		if (param1 instanceof Scaler) {
			if (param2 instanceof Scaler) { // Scaler * Scaler -> Scaler
			...
			} else if (param2 instanceof Matrix) // Vector * Matrix -> Vector
			{
				...
				if (lhs.getNumEles() != rhs.getNumRows())
					throw new ParseException("Multiply Matrix , Vector: Miss match in sizes (" + lhs.getNumEles() + ","
							+ rhs.getNumRows() + ")!");
				for (int i = 0; i < rhs.getNumCols(); ++i) {
					Object val = super.mul(lhs.getEle(0), rhs.getEle(0, i));
					for (int j = 1; j < rhs.getNumRows(); ++j)
						val = add.add(val, super.mul(lhs.getEle(j), rhs.getEle(j, i)));
					res.setEle(i, val);
				}
			} else {
				throw new ParseException("Sorry I don't know how to multiply a vector by a tensor");
			...
			} else if (param2 instanceof MVector) // Matrix * Vector -> Vector
			{
				...
				if (lhs.getNumCols() != rhs.getNumEles())
					throw new ParseException("Mat * Vec: Miss match in sizes (" + lhs.getNumCols() + ","
							+ rhs.getNumEles() + ") when trying to add vectors!");
			...
			} else if (param2 instanceof Matrix) // Matrix * Matrix -> Matrix
			{
				...
				if (lhs.getNumCols() != rhs.getNumRows())
					throw new ParseException("Multiply matrix,matrix: Miss match in number of dims ("
				...
				for (int i = 0; i < lnr; ++i)
					for (int j = 0; j < rnc; ++j) {
						...
						for (int k = 1; k < lnc; ++k)
							val = add.add(val, mul(ldata[i][k], rdata[k][j]));
						resdata[i][j] = val;
					}
			} else
				// Tensor res
