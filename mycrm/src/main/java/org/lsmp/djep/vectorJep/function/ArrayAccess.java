 ...
 * is handled by parent VEle class. For examples <code>
 * a=[1,2,3];
 * a[2]=4;
 * b=[[1,2],[3,4]];
 * b[2,1]=5; 
 * </code>
 * 
 * @author Richard Morris TODO implement setting slices a[3:5]=[3,4,5]
 *         a[1,]=[1,3]
 */
	...
	public void set(EvaluatorI pv, Node node, Object value) throws ParseException {
		...
		if (varVal instanceof MVector) {
			if (indicies instanceof Number) {
				...
				if (vec.getDim().equals(Dimensions.ONE)) {
					...
					if (d1 < 1 || d1 > ((MVector) varVal).getNumEles())
						throw new ParseException("ArrayAccess: array index " + d1 + " out of range 1.."
								+ varVal.getDim());
			...
			if (indicies instanceof MVector) {
				...
				throw new ParseException("ArrayAccess:Bad second argument, expecting [i,j] " + indicies.toString());
		...
		throw new ParseException("ArrayAccess: requires a vector matrix or tensor for first argument it has "
				+ varVal.toString());
