...
/**
 * A class to represent a set of dimensions. Might be 1 for 0-dimensional
 * numbers. [3] for a 3D vector [3,3] for a matrix
 * 
 * @author rich Created on 25-Oct-2003
 */
public class Dimensions {
	...
	private Dimensions(int d1, int d2) {
		dims = new int[] { d1, d2 };
	}
	...
	/**
	 * Factory method returns a Dimension for vector of given length. For 1,2,3
	 * returns ONE,TWO or THREE otherwise create a new object.
	 */
	...
	public static Dimensions valueOf(int dims[]) {
		if (dims.length == 1)
			return valueOf(dims[0]);
		if (dims.length == 2)
			return valueOf(dims[0], dims[1]);
		return new Dimensions(dims);
	}

	/**
	 * get the first dimension, 1 for numbers, or the length of a vector. for a
	 * matrix [[1,2,3],[4,5,6]] first dim is number of rows eg 2
	 */
	...
	/**
	 * get the last dimension, 1 for numbers, or the length of a vector. for a
	 * matrix [[1,2,3],[4,5,6]] last dim is number of cols eg 3
	 */
	...
	public boolean equals(Dimensions dims2) {
		if (dims2 == null)
			return false;
		if (dims.length != dims2.dims.length)
			return false;
		for (int i = 0; i < dims.length; ++i) {
			if (dims[i] != dims2.dims[i])
				return false;
		}
		return true;
	}
	...
	/**
	 * apparently your should always override hashcode when you override equals
