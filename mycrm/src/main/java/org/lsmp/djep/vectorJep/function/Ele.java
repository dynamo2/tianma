 ...
 * indexing convention with indices starting from 1 rather than the computer
 * science convention with indices starting from 0. Hence <code>
 * a = [1,2,3,4];
 * ele(a,1); // returns 1
 * m = [[1,2],[3,4]];
 * ele(m,[2,2]); // return 4
 * </code>
 * 
 * New parser feature allow a[] notation to be used. <code>
 * a=[1,2,3,4];
 * a[3]; // returns 3
 * b=[[1,2],[3,4]];
 * b[1,2]; // returns 2
			...
			if (param2 instanceof Scaler) {
				...
				if (vec.getDim().equals(Dimensions.ONE)) {
					...
					if (d1 < 1 || d1 > ((MVector) param1).getNumEles())
						throw new ParseException("ArrayAccess: array indices " + d1 + " out of range 1.."
								+ param1.getDim());
			...
			if (param2 instanceof MVector) {
				...
				if (vec.getDim().equals(Dimensions.TWO)) {
					...
					if (d1 < 1 || d1 > ((Matrix) param1).getNumRows() || d2 < 1 || d2 > ((Matrix) param1).getNumCols())
						throw new ParseException("ArrayAccess: array indices " + d1 + ", " + d2 + " out of range 1.."
								+ param1.getDim());
				...
				throw new ParseException("Bad second argument to ele, expecting [i,j] " + param2.toString());
		} else if (param1 instanceof Tensor) {
			throw new ParseException("Sorry don't know how to find elements for a tensor");
		} else
			throw new ParseException("ele requires a vector matrix or tensor for first argument it has "
					+ param1.toString());
		return res;
	}
