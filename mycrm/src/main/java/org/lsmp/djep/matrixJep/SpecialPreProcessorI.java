...
public interface SpecialPreProcessorI {
	/**
	 * Subverts the preprocessing stage. Preprocessing performs a number of
	 * operations:
	 * <ol>
	 ...
	 * </ol>
	 * In general the first step in preprocessing is to run the preprocessor on
	 * the children of the node. This can be done using
	 * 
	 * <pre>
	 * MatrixNodeI children[] = visitor.visitChildrenAsArray(node, null);
	 * </pre>
	 * 
	 * The final step is to construct a node of the correct type. The
	 * MatrixNodeFactory argument has a number of methods to do this. For
	 * example
	 * 
	 * <pre>
	 ...
	 * 
	 * @param node
	 *            the top node of the tree representing the function and its
	 *            arguments.
	 * @param visitor
	 *            A reference to the preprocessing visitor.
	 * @param jep
	 *            A reference of the MatrixJep instance.
	 * @param nf
	 ...
	 */
	public MatrixNodeI preprocess(ASTFunNode node, MatrixPreprocessor visitor, MatrixJep jep, MatrixNodeFactory nf)
			throws ParseException;
