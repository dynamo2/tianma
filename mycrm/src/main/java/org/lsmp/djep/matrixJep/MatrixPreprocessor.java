 ...
 * <li>For the List opperator it finds the dimensions and returns a ASTTensor.
 * <li>For the Hat opperator it finds the dimension and returns a Power or Wedge
 * opperator.
 * </ul>
...
public class MatrixPreprocessor implements ParserVisitor {
	 ...
	 *            Top node of tree.
	 * @param mdjep
	 *            Reference to MatrixJep instance
	...
	/**
	 * Returns an array of matrix nodes which are the results of visiting each
	 * child.
	 */
	...
	public Object visit(ASTFunNode node, Object data) throws ParseException {
		...
		if (node.getPFMC() instanceof CommandVisitorI)
			throw new IllegalArgumentException(
					"MatrixPreprocessor: encountered and instance of CommandVisitorI  for function " + node.getName());
	...
	public Object visitOp(ASTFunNode node, Object data) throws ParseException {
		...
		if (pfmc instanceof BinaryOperatorI) {
			if (node.jjtGetNumChildren() != 2)
				throw new ParseException("Operator " + node.getOperator().getName()
						+ " must have two elements, it has " + children.length);
			...
			if (children.length != 1)
				throw new ParseException("Operator " + node.getOperator().getName()
						+ " must have one elements, it has " + children.length);
			...
			// throw new
			// ParseException("Operator must be unary or binary. It is "+op);
