...
/**
 * The trapezium rule for approximation to a definite integral. h * (y0 + yn +
 * 2(y1+y2+...+y_(n-1)) /2 where h = (xn-x0)/n, yi = f(xi)
 * Trapezium(x^2,x,0,10,0.5) finds an approximation for int(x^2) where x runs
 * from 0 to 10 in steps of h=0.5.
 * 
...
public class Trapezium extends SumType {
	...
	public Object evaluate(Object elements[]) throws ParseException {
		...
		Object ret = mul.mul(HALF, add.add(elements[0], elements[elements.length - 1]));
