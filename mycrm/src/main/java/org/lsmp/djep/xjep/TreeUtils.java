...
/**
 * A set of Utility functions for working with JEP expression trees. Main
 * methods are
 * <ul>
 * <li> {@link #isConstant isConstant} test if its a constant. Many other is...
 * methods.
 * <li> {@link #getValue getValue} extracts the value from a node without needing
 * to cast and check types.
 * </ul>
 * 
 * @author rich
 */
public class TreeUtils {
	/**
	 * Real zero. Note that this is a Double, if a different number format is
	 * needed then this class should be sub-classed.
	 */
	 ...
	 *             if given something which is not an ASTConstant
	 */
	...
	/*
	 * public PostfixMathCommandI getPfmc(String name) { return
	 * (PostfixMathCommandI) myFunTab.get(name); }
	 */
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if given something which is not an ASTConstant with a Double
	 *             value
	 */
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if given something which is not an ASTConstant with a Complex
	 *             value
	 */
	...
	/**
	 * returns true if node is a ASTConstant with Double value representing an
	 * integer.
	 */
	...
	/**
	 * returns true if node is a ASTConstant with value Double(0) or
	 * Complex(0,0)
	 */
	public boolean isZero(Node node) {
		return (isReal(node) && (((ASTConstant) node).getValue().equals(ZERO)))
				|| (isComplex(node) && (((Complex) ((ASTConstant) node).getValue()).equals(CZERO, 0.0)));
	}
	 ...
	 * 
	 * @param tol
	 *            tolerance for testing for zero
	 */

	public boolean isZero(Node node, double tol) {
		return (isReal(node) && ((((ASTConstant) node).getValue().equals(ZERO))) || Math.abs(doubleValue(node)) < tol)
				|| (isComplex(node) && (((Complex) ((ASTConstant) node).getValue()).equals(CZERO, tol)));
	}

	/**
	 * returns true if node is a ASTConstant with value Double(1) or
	 * Complex(1,0)
	 */
	public boolean isOne(Node node) {
		return (isReal(node) && (((ASTConstant) node).getValue().equals(ONE)))
				|| (isComplex(node) && (((Complex) ((ASTConstant) node).getValue()).equals(CONE, 0.0)));
	}
	...
	public boolean isMinusOne(Node node) {
		return (isReal(node) && (((ASTConstant) node).getValue().equals(MINUSONE)))
				|| (isComplex(node) && (((Complex) ((ASTConstant) node).getValue()).equals(CMINUSONE, 0.0)));
	}

	/**
	 * returns true if node is a ASTConstant with a Infinite component TODO do
	 * proper treatment of signed infinity
	 */
	...
	public boolean isNegative(Node node) {
		return isReal(node) && (((Double) ((ASTConstant) node).getValue()).doubleValue() < 0.0);
	}
	...
	public static Node copyChildrenIfNeeded(Node node, Node children[]) throws ParseException {
		...
		if (n != children.length)
			throw new ParseException(
					"copyChildrenIfNeeded: umber of children of node not the same as supplied children");
		for (int i = 0; i < n; ++i)
