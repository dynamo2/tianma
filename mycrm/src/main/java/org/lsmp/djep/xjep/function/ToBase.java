 ...
 * decimal place. toHex(val) converts to base 16 toHex(val,3) converts to base
 * 16 with 3 hex digits after decimal place. A prefix can be specified in the
 * constructor. If set this will be appended to the number (after minus sign for
 * negative values).
 * 
...
public class ToBase extends PostfixMathCommand {
	...
	public ToBase(int base) {
		if (base < Character.MIN_RADIX || base > Character.MAX_RADIX)
			throw new IllegalArgumentException("base must be between " + Character.MIN_RADIX + " and "
					+ Character.MIN_RADIX);
	...
	/**
	 * Constructor with specified base and a given prefix. For example 0x to
	 * proceed hexadecimal numbers.
	 * 
	 * @param base
	 *            the base to use
	 * @param prefix
