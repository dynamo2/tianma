...
/**
 * The Simpson rule for approximation to a definite integral. h * (y0 + yn +
 * 4(y1+y3+...+y_(n-1)) + 2(y2+y4+...+y_(n-2)) ) /3 where h = (xn-x0)/n, yi =
 * f(xi) Simpson(x^2,x,0,10,0.5) finds an approximation for int(x^2) where x
 * runs from 0 to 10 in steps of h=0.5.
 * 
	...
	public Object evaluate(Object elements[]) throws ParseException {
		...
		for (int i = 1; i < elements.length - 1; ++i) {
			...
			if (i % 2 == 0)
				...
				ret = add.add(ret, mul.mul(FOUR, elements[i]));
		}
