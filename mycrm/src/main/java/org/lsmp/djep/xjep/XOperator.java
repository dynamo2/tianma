 ...
 * <li>The number of arguments NO_ARGS 0, UNARY 1 (eg UMINUS -x), BINARY 2 (eq
 * x+y), and NARY either 3 ( a>b ? a : b) or unspecified like a list [x,y,z,w].
 * <li>The binging of the operator, LEFT 1+2+3 -> (1+2)+3 or RIGHT 1=2=3 ->
 * 1=(2=3).
 ...
 * <li>Comparative operators can be REFLEXIVE, SYMMETRIC, TRANSITIVE or
 * EQUIVILENCE which has all three properties.
 * <li>A reference to a PostfixtMathCommandI object which is used to evaluate an
 * equation containing the operator.
 * </ul>
 * various is... and get... methods are provided to query the properties of the
 * opperator.
 * 
...
public class XOperator extends Operator {
	...
	/**
	 * postfix operators x%, if neiter prefix and postif then infix, if both
	 * trifix like x?y:z
	 **/
	...
	/**
	 * For non commutative operators printing can be determined by the left or
	 * right binding. For example (a-b)-c is printed as a-b-c. But a/b/c could
	 * be ambiguous so (a/b)/c is printed with brackets.
	 */
	 ...
	 * 
	 * @param name
	 *            printable name of operator
	 * @param pfmc
	 *            postfix math command for opperator
	 * @param flags
	 *            set of flags defining the porperties of the operator.
	 */
	 ...
	 * 
	 * @param name
	 *            name of operator, must be unique, used when describing
	 *            operator
	 * @param symbol
	...
	/**
	 * precedence of operator, 0 is most tightly bound, so prec("*") <
	 * prec("+").
	 */
	...
	/**
	 * For composite operators like a-b which is really a+(-b) there is a root
	 * operator and an inverse operator
	 **/
	...
	public Operator getBinaryInverseOp() {
		return binaryInverseOperator;
	}

	/**
	 * When parsing how is x+y+z interpreted. Can be Operator.LEFT x+y+z ->
	 * (x+y)+z or Operator.RIGHT x=y=z -> x=(y=z).
	 */
	...
	public final boolean isAssociative() {
		return ((flags & ASSOCIATIVE) == ASSOCIATIVE);
	}

	public final boolean isCommutative() {
		return ((flags & COMMUTATIVE) == COMMUTATIVE);
	}

	public final boolean isBinary() {
		return ((flags & 3) == BINARY);
	}

	public final boolean isUnary() {
		return ((flags & 3) == UNARY);
	}
	...
	public final boolean isTransitive() {
		return ((flags & TRANSITIVE) == TRANSITIVE);
	}

	public final boolean isSymmetric() {
		return ((flags & SYMMETRIC) == SYMMETRIC);
	}

	public final boolean isReflexive() {
		return ((flags & REFLEXIVE) == REFLEXIVE);
	}

	public final boolean isEquivilence() {
		return ((flags & EQUIVILENCE) == EQUIVILENCE);
	}

	public final boolean isPrefix() {
		return ((flags & PREFIX) == PREFIX);
	}

	public final boolean isSufix() {
		return ((flags & SUFIX) == SUFIX);
	}

	public final boolean isComposite() {
		return ((flags & COMPOSITE) == COMPOSITE);
	}

	public final boolean isSelfInverse() {
		return ((flags & SELF_INVERSE) == SELF_INVERSE);
	}

	public final boolean useBindingForPrint() {
		return ((flags & USE_BINDING_FOR_PRINT) == USE_BINDING_FOR_PRINT);
	}
