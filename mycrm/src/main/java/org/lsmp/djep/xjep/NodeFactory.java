...
/**
 * This class is used to create nodes of specified types. It can be sub-classed
 * to change the nature of how nodes are constructed. Generally there are two
 * methods for creating nodes, methods which take an existing node and methods
 * which take the components.
 * 
...
public class NodeFactory {
	...
	/**
	 * Creates an ASTConstant node with specified value. This method should be
	...
	/**
	 * Creates a ASTConstant whose value of applying the operator to its
	 * arguments.
	 */
	public ASTConstant buildConstantNode(PostfixMathCommandI pfmc, Node children[]) throws IllegalArgumentException,
			ParseException {
		/*
		 * Stack stack = new Stack(); for(int i=0;i<children.length;++i) {
		 * if(!(children[i] instanceof ASTConstant)) throw new
		 * ParseException("buildConstantNode: arguments must all be constant nodes"
		 * ); stack.push(((ASTConstant) children[i]).getValue()); }
		 * pfmc.setCurNumberOfParameters(children.length); pfmc.run(stack);
	...
	/**
	 * Creates a ASTConstant whose value of applying binary operator to its
	 * arguments.
	 */
	public ASTConstant buildConstantNode(Operator op, Node child1, Node child2) throws IllegalArgumentException,
			ParseException {
		return buildConstantNode(op.getPFMC(), new Node[] { child1, child2 });
	}

	/**
	 * Creates a ASTConstant whose value of applying a unary operator to its
	 * arguments.
	 */
	...
	/**
	 * creates a new ASTVarNode with a given variable. This method should be
	 * sub-classed
	 * 
	 * @throws ParseException
	 */
	...
	/**
	 * An unfinished node. Caller has responsibility for filling in the
	 * children.
	 */
	...
	/**
	 * Builds a function with n arguments and same fun as specified in
	 * arguments.
	 * 
	 * @param node
