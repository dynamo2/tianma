...
/**
 * Prints an expression. Prints the expression with lots of brackets.
 * <tt>((-1.0)/sqrt((1.0-(x^2.0))))</tt>. To use
 * 
 * <pre>
 * XJep j = ...; Node in = ...;
 * j.print(in,"x");
 ...
 * @since Dec 04 and NumberFormat object can be supplied to modify printing of
 *        numbers.
...
public class PrintVisitor extends ErrorCatchingVisitor {
	...
	/**
	 * Creates a visitor to create and print string representations of an
	 * expression tree.
	 **/
	...
	/**
	 * Prints the tree descending from node with lots of brackets or specified
	 * stream.
	 * 
	 * @see XJep#println(Node, PrintStream)
	 **/
	...
	/**
	 * This interface specifies the method needed to implement a special print
	 * rule. A special rule must implement the append method, which should call
	 * pv.append to add data to the output. For example
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * @author Rich Morris Created on 21-Feb-2004
	 */
	...
	/**
	 * Add a string to buffer. Classes implementing PrintRulesI should call this
	 * add the
	 */
	...
	/**
	 * Adds a special print rule to be added for a given operator. TODO Allow
	 * special rules for other functions, i.e. not operators.
	 */
	...
	private boolean testMid(XOperator top, Node rhs) {
		if ((mode & FULL_BRACKET) != 0) {
			...
			} else if (top.getPrecedence() > rhsop.getPrecedence()) // 1+(2*3)
																	// -> 1+2*3
				return false;
			else
				return true;
	...
	private boolean testRight(XOperator top, Node rhs) {
		if ((mode & FULL_BRACKET) != 0) {
			...
			if (top == rhsop) {
				...
				if (top.getBinding() == XOperator.LEFT && top.isAssociative()) // 1+(2-3)
																				// ->
																				// 1+2-3)
					return false; // a+(b-c) -> a+b-c
	...
	/**
	 * Set printing mode. In full bracket mode the brackets each element in the
	 * tree will be surrounded by brackets to indicate the tree structure. In
	 * the default mode, (full bracket off) the number of brackets is minimised
	 * so (x+y)+z will be printed as x+y+z.
	 * 
	 * @param mode
	...
	/**
	 * Sets the maximum length printed per line. If the value is not -1 then the
	 * string will be broken into chunks each of which is less than the max
	 * length.
	 * 
	 * @param i
	 *            the maximum length
