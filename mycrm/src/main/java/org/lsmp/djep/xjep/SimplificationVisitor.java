 ...
 * 
 * <p>
 * Its intended to completly rewrite this class to that simplification rules can
 * be specified by strings in a way similar to DiffRulesI. It also would be nice
 * to change the rules depending on the type of arguments, for example matrix
 * multiplication is not commutative. But some of the in built rules exploit
 * commutativity.
 * 
 * @author Rich Morris Created on 20-Jun-2003 TODO cope with 'a - (-1) * b' TODO
 *         cope with '0 - uminus(b)' TODO cope with simplifying complex numbers
 */
...
public class SimplificationVisitor extends DoNothingVisitor {
	...
	public Node simplifyTripple(XOperator op, Node lhs, Node rhs) throws ParseException {
		...
		if (tu.isConstant(lhs) && tu.isBinaryOperator(rhs)) {
			...
			if (rhsOp.isComposite())
				rhsRoot = (XOperator) rhsOp.getRootOp();
			else
				rhsRoot = rhsOp;
			...
			if (tu.isConstant(rhsChild1)) {
				...
				if (op == rhsOp)
					...
					op2 = (XOperator) rootOp.getBinaryInverseOp();

				// 2 + ~( 3 + ~x ) -> (2+~3) + ~~x
				if (rootOp == rhsRoot && rootOp.isAssociative()) {
					Node newnode = simplifyBuiltOperatorNode(op2, nf.buildConstantNode(op, lhs, rhsChild1), rhsChild2);
					return newnode;
				}

				if (op.isDistributiveOver(rhsRoot)) // 2 * (3 + ~x) -> (2 * 3) +
													// ~(2 @ x)
				{
					Node newnode = simplifyBuiltOperatorNode(rhsOp, nf.buildConstantNode(op, lhs, rhsChild1),
			...
			if (tu.isConstant(rhsChild2)) {
				...
				if (op == rhsOp)
					...
					op2 = rootOp.getBinaryInverseOp();

				if (rootOp == rhsRoot && rootOp.isCommutative() && rootOp.isAssociative()) {
					Node newnode = simplifyBuiltOperatorNode(op, nf.buildConstantNode(op2, lhs, rhsChild2), rhsChild1);
					return newnode;
				}

				if (op.isDistributiveOver(rhsRoot)) // 2 * (x + ~3) -> (2 * x) +
													// ~(2 * 3)
				{
					Node newnode = simplifyBuiltOperatorNode(rhsOp, simplifyBuiltOperatorNode(op, lhs, rhsChild1),
		...
		if (tu.isBinaryOperator(lhs) && tu.isConstant(rhs)) {
			...
			XOperator lhsRoot;
			if (lhsOp.isComposite())
				lhsRoot = (XOperator) lhsOp.getRootOp();
			else
				lhsRoot = lhsOp;
			...
			if (tu.isConstant(lhsChild1)) {
				...
				if (rootOp == lhsRoot && rootOp.isAssociative() && rootOp.isCommutative()) {
					Node newnode = simplifyBuiltOperatorNode(lhsOp, nf.buildConstantNode(op, lhsChild1, rhs), lhsChild2);
					return newnode;
				}

				// (2 + ~x) * 3 --> (2*3) +~ (x*3)
				if (op.isDistributiveOver(lhsRoot)) {
					Node newnode = simplifyBuiltOperatorNode(lhsOp, nf.buildConstantNode(op, lhsChild1, rhs),
			...
			if (tu.isConstant(lhsChild2)) {
				...
				if (rootOp == lhsRoot && rootOp.isAssociative()) {
					...
					Node newnode = simplifyBuiltOperatorNode(lhsOp, lhsChild1,
				...
				// (x + ~2) * 3 -> (x*3) + ~(2*3)
				if (op.isDistributiveOver(lhsRoot)) {
					Node newnode = simplifyBuiltOperatorNode(lhsOp, simplifyBuiltOperatorNode(op, lhsChild1, rhs),
	...
	public Node simplifyAdd(Node lhs, Node rhs) throws ParseException {
		if (tu.isInfinity(lhs)) { // Inf + Inf -> NaN TODO not correct for
									// signed infinity
		...
		{
			Node newnode = nf.buildOperatorNode(opSet.getSubtract(), rhs, nf.buildConstantNode(opSet.getUMinus(), lhs));
			return newnode;
		}
		if (tu.isNegative(rhs)) // x + -3 -> x - 3
		{
			Node newnode = nf.buildOperatorNode(opSet.getSubtract(), lhs, nf.buildConstantNode(opSet.getUMinus(), rhs));
			return newnode;
		}
		return null;
	...
	public Node simplifySubtract(Node lhs, Node rhs) throws ParseException {
		...
		{
			Node newnode = simplifyBuiltOperatorNode(opSet.getAdd(), lhs, nf.buildConstantNode(opSet.getUMinus(), rhs));
			return newnode;
		}

		if (tu.getOperator(rhs) == opSet.getUMinus()) {
			Node newnode = simplifyBuiltOperatorNode(opSet.getAdd(), lhs, rhs.jjtGetChild(0));
			return newnode;
		}
		/*
		 * if(tu.getOperator(rhs)==opSet.getMultiply()) {
		 * if(tu.isNegative(rhs.jjtGetChild(0))) // a - (-2) * b -> a + 2 * b {
		 * Node newnode = simplifyBuiltOperatorNode( opSet.getAdd(), lhs,
		 * nf.buildOperatorNode( opSet.getMultiply(), nf.buildConstantNode(
		 * opSet.getUMinus(),rhs.jjtGetChild(0)), rhs.jjtGetChild(1))); return
		 * newnode; } }
		 */
		return null;
		// return nf.buildOperatorNode(((ASTOpNode)
		// node).getOperator(),lhs,rhs);
	...
	public Node simplifyPower(Node child1, Node child2) throws ParseException {
		...
		if (tu.isConstant(child2) && tu.getOperator(child1) == opSet.getPower()) {
			if (tu.isConstant(child1.jjtGetChild(1))) {
				/* (x^3)^4 -> x^(3*4) */
				return nf.buildOperatorNode(opSet.getPower(), child1.jjtGetChild(0),
						nf.buildConstantNode(opSet.getMultiply(), child1.jjtGetChild(1), child2));
			}
		}
		return null;
		// return nf.buildOperatorNode(((ASTOpNode)
