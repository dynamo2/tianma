 ...
 * </pre>
 * 
 * The names of the variables used inside the function depends on the number of
 * arguments:
 * <ul>
 * <li>One argument variable must be x:
 * <tt>new MacroFunction("sec",1,"1/cos(x)",j)</tt></li>
 * <li>Two arguments variables must be x or y:
 * <tt>new MacroFunction("myPower",2,"x^y",j)</tt></li>
 * <li>Three or more arguments variables must be x1, x2, x3,...:
 * <tt>new MacroFunction("add3",3,"x1+x2+x3",j)</tt></li>
 * </ul>
 * 
...
public class MacroFunction extends PostfixMathCommand {
	...
	/**
	 * Create a function specified by a string. For example
	 * <tt>new MacroFunction("sec",1,"1/cos(x)",tu)</tt> creates the function
	 * for sec. Variable names must be x,y for 1 or 2 variables or x1,x2,x3,..
	 ...
	 */
	public MacroFunction(String inName, int nargs, String expression, XJep jep) throws IllegalArgumentException,
			ParseException {
		super();
		name = inName;
		...
		if (numberOfParameters == 0) {
			...
			vars = new Variable[] { mySymTab.addVariable("x", null), mySymTab.addVariable("y", null) };
		} else {
