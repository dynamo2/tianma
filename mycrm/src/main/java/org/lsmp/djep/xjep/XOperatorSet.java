...
/**
 * An OperatorSet where the operators have information about their commutativity
 * etc.
 * 
 * @see XOperator
 * @author Rich Morris Created on 26-Jul-2003
 */
public class XOperatorSet extends OperatorSet {
	...
	private void annotateOperators(OperatorSet o) {
		...
		OP_AND = new XOperator(o.getAnd(), XOperator.BINARY + XOperator.LEFT + XOperator.COMMUTATIVE
				+ XOperator.ASSOCIATIVE + XOperator.USE_BINDING_FOR_PRINT);
		OP_OR = new XOperator(o.getOr(), XOperator.BINARY + XOperator.LEFT + XOperator.COMMUTATIVE
				+ XOperator.ASSOCIATIVE);
		OP_NOT = new XOperator(o.getNot(), XOperator.UNARY + XOperator.RIGHT + XOperator.PREFIX
				+ XOperator.SELF_INVERSE);

		OP_ADD = new XOperator(o.getAdd(), XOperator.BINARY + XOperator.LEFT + XOperator.COMMUTATIVE
				+ XOperator.ASSOCIATIVE);
		OP_SUBTRACT = new XOperator(o.getSubtract(), XOperator.BINARY + XOperator.LEFT + XOperator.COMPOSITE
				+ XOperator.USE_BINDING_FOR_PRINT);
		OP_UMINUS = new XOperator(o.getUMinus(), XOperator.UNARY + XOperator.RIGHT + XOperator.PREFIX
				+ XOperator.SELF_INVERSE);

		OP_MULTIPLY = new XOperator(o.getMultiply(), XOperator.BINARY + XOperator.LEFT + XOperator.COMMUTATIVE
				+ XOperator.ASSOCIATIVE);
		...
		OP_UDIVIDE = new XOperator("UDivide", "^-1", null, XOperator.UNARY + XOperator.RIGHT + XOperator.PREFIX
				+ XOperator.SELF_INVERSE);
		...
		// OP_RANGE = new
		// XOperator(o.getRange(),XOperator.NARY+XOperator.RIGHT); //
		setPrecedenceTable(new Operator[][] { { OP_UMINUS }, { OP_NOT }, { OP_POWER },
				{ OP_MULTIPLY, OP_DIVIDE, OP_MOD, OP_DOT, OP_CROSS }, { OP_ADD, OP_SUBTRACT }, { OP_LT, OP_LE },
		...
		((XOperator) OP_UMINUS).setDistributiveOver(OP_ADD); // -(a+b) -> (-a) +
																// (-b)
		((XOperator) OP_UMINUS).setDistributiveOver(OP_SUBTRACT); // -(a-b) ->
																	// (-a) -
																	// (-b)

		((XOperator) OP_MULTIPLY).setDistributiveOver(OP_ADD); // a*(b+c) -> a*b
																// + a*c
		((XOperator) OP_MULTIPLY).setDistributiveOver(OP_SUBTRACT); // a*(b-c)
																	// -> a*b -
																	// a*c
		((XOperator) OP_MULTIPLY).setDistributiveOver(OP_UMINUS); // a*(-b) ->
																	// -(a*b)
	}

	/**
	 * Creates the operator set from a given set. Will use the names and pfmc's
	 * but adds info about the operators properties. Note changes pfmc for =
	 * from Assign to XAssign
	 */
	...
	/**
	 * Sets the precedences of the operators according to order in the supplied
	 * array. For example
	 * 
	 * <pre>
	 * setPrecedenceTable(new Operator[][] { { OP_UMINUS }, { OP_NOT }, { OP_MUL, OP_DIV, OP_MOD }, { OP_PLUS, OP_MINUS },
	...
	/**
	 * Prints all the operators, with verbose representations of each operators
	 * properties.
	 * 
	 */
