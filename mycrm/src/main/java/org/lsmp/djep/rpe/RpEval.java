...
/**
 * A fast evaluation algorithm for equations over Doubles, does not work with
 * vectors or matricies. This is based around reverse polish notation and is
 * optimised for speed at every opportunity.
 * <p>
 * To use do
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The evaluate method executes these methods sequentially using a stack and
 * returns the last object on the stack.
 * <p>
 * A few cautionary notes: Its very unlikely to be thread safe. It only works
 * over doubles expressions with complex numbers or strings will cause problems.
 ...
 * <li>Each operator/function is hand coded. To extend functionality you will
 * have to modify the source.</li>
 * </ul>
 * 
...
public final class RpEval implements ParserVisitor {
	 ...
	 * </pre>
	 * 
	 * and the stack is the current data used for calculations. Data for
	 * Variables is stored in vars and references to the Variables in varRefs.
	 */
	...
	public final Object visit(ASTFunNode node, Object data) throws ParseException {
		...
		if (node.isOperator()) {
			...
			if (op == opSet.getAdd()) {
				...
				scalerStore.decStack();
				curCommandList.addCommand(EQ);
				return null;
			} else if (op == opSet.getNE()) {
				scalerStore.decStack();
				curCommandList.addCommand(NE);
				return null;
			} else if (op == opSet.getLT()) {
				scalerStore.decStack();
				curCommandList.addCommand(LT);
				return null;
			} else if (op == opSet.getGT()) {
				scalerStore.decStack();
				curCommandList.addCommand(GT);
				return null;
			} else if (op == opSet.getLE()) {
				scalerStore.decStack();
				curCommandList.addCommand(LE);
				return null;
			} else if (op == opSet.getGE()) {
				scalerStore.decStack();
				curCommandList.addCommand(GE);
				return null;
			} else if (op == opSet.getAnd()) {
				scalerStore.decStack();
				curCommandList.addCommand(AND);
				return null;
			} else if (op == opSet.getOr()) {
				scalerStore.decStack();
				curCommandList.addCommand(OR);
				return null;
			} else if (op == opSet.getNot()) {
				// scalerStore.decStack();
				curCommandList.addCommand(NOT);
				return null;
			} else if (op == opSet.getDivide()) {
				scalerStore.decStack();
				curCommandList.addCommand(DIV);
				return null;
			} else if (op == opSet.getMod()) {
				scalerStore.decStack();
				curCommandList.addCommand(MOD);
				return null;
	...
	public final double evaluate(RpCommandList comList) {
		...
		for (short commandNum = 0; commandNum < num; ++commandNum) {
			...
			case CONST:
				...
				break;
			case VAR:
				scalerStore.stack[scalerStore.sp++] = scalerStore.vars[aux1];
				break;
