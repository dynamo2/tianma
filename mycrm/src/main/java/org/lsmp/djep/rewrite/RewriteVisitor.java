 ...
 * 
 * <p>
 * Its intended to completely rewrite this class to that simplification rules
 * can be specified by strings in a way similar to DiffRulesI. It also would be
 * nice to change the rules depending on the type of arguments, for example
 * matrix multiplication is not commutative. But some of the in built rules
...
public class RewriteVisitor extends DoNothingVisitor {
	...
	public Node rewrite(Node node, XJep xjep, RewriteRuleI inrules[], boolean simplify) throws ParseException,
