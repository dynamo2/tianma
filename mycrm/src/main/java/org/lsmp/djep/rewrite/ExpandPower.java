...
public class ExpandPower implements RewriteRuleI {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.lsmp.djep.xjep.RewriteRuleI#test(org.nfunk.jep.Node,
	 * org.nfunk.jep.Node[])
	 */
	public boolean test(ASTFunNode node, Node[] children) {
		...
		if (opSet.getPower() == op) {
			if (tu.getOperator(children[0]) == opSet.getAdd() || tu.getOperator(children[0]) == opSet.getSubtract()) {
	...
	public Node apply(ASTFunNode node, Node[] children) throws ParseException {
		...
		if (lhsOp == opSet.getAdd() || lhsOp == opSet.getSubtract()) { /*
																		 ...
																		 */
			if (n == 0)
				return nf.buildConstantNode(new Double(1));
			if (n == 1)
				return children[0];
			...
			if (n == 2) {
				...
				vals[1] = nf.buildOperatorNode(
						...
						nf.buildOperatorNode(
								opSet.getMultiply(),
								nf.buildOperatorNode(opSet.getPower(), xj.deepCopy(sub1),
										nf.buildConstantNode(new Double(n - 1))), xj.deepCopy(sub2)));
			}
			/* n * a * b^(n-1) */
			if (n >= 3) {
				vals[n - 1] = nf.buildOperatorNode(
						...
						nf.buildOperatorNode(
								opSet.getMultiply(),
								xj.deepCopy(sub1),
								nf.buildOperatorNode(opSet.getPower(), xj.deepCopy(sub2),
			...
			sums[n] = vals[n];
			for (int i = n - 1; i >= 0; --i) {
				sums[i] = nf.buildOperatorNode(lhsOp, vals[i], sums[i + 1]);
			}
			return sums[0];
		}
