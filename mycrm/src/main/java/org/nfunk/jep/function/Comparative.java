 ...
 * and == work. For other types care might be needed.
 * 
 * Complex numbers are compared using a tolerance which can be set using
 * setTolerance().
 * 
...
public class Comparative extends PostfixMathCommand {
	...
	/*
	 * old code if ((param1 instanceof Complex) || (param2 instanceof Complex))
	 * throw new ParseException(">= not defined for complex numbers"); if
	 * ((param1 instanceof Number) && (param2 instanceof Number)) { double x =
	 * ((Number)param1).doubleValue(); double y =
	 * ((Number)param2).doubleValue(); int r = (x>=y) ? 1 : 0; return new
	 * Double(r); } throw new
	 * ParseException(">= not defined for object of type "
	 * +param1.getClass().getName()+" and "+param2.getClass().getName()); }
	 * 
	 * { int r;
	 * 
	 * switch (id) { case NE: r = ((Complex)param1).equals((Complex)param2,
	 * tolerance) ? 0 : 1; break; case EQ: r =
	 * ((Complex)param1).equals((Complex)param2, tolerance) ? 1 : 0; break;
	 * default: throw new ParseException("Relational operator type error"); }
	 * 
	 * inStack.push(new Double(r));//push the result on the inStack } else if
	 * ((param1 instanceof Number) && (param2 instanceof Number)) { double x =
	 * ((Number)param1).doubleValue(); double y =
	 * ((Number)param2).doubleValue(); int r;
	 * 
	 * switch (id) { case LT: r = (x<y) ? 1 : 0; break; case GT: r = (x>y) ? 1 :
	 * 0; break; case LE: r = (x<=y) ? 1 : 0; break; case GE: r = (x>=y) ? 1 :
	 * 0; break; case NE: r = (x!=y) ? 1 : 0; break; case EQ: r = (x==y) ? 1 :
	 * 0; break; default: throw new
	 * ParseException("Unknown relational operator"); }
	 * 
	 * inStack.push(new Double(r));//push the result on the inStack } else if
	 * ((param1 instanceof String) && (param2 instanceof String)) { int r;
	 * 
	 * switch (id) { case NE: r = ((String)param1).equals((String)param2) ? 0 :
	 * 1; break; case EQ: r = ((String)param1).equals((String)param2) ? 1 : 0;
	 * break; default: throw new
	 * ParseException("Relational operator type error"); }
	 * 
	 * inStack.push(new Double(r));//push the result on the inStack } else {
