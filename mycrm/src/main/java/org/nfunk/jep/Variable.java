...
/**
 * Information about a variable. Each variable has a name, a value. There is a
 * flag to indicate whether it is a constant or not (constants cannot have their
 * value changed). There is also a flag to indicate whether the value of the
 * variable is valid, if the variable is initialised without a value then its
 * value is said to be invalid.
 * <p>
 * 
...
public class Variable extends Observable {
	...
	/**
	 * Constructors are protected. Variables should only be created through the
	 * associated {@link VariableFactory} which are in turned called by
	 * {@link SymbolTable}.
	 */
	 ...
	 * <p>
	 * This method call java.util.Observable.notifyObservers() to indicate to
	 * anyone interested that the value has been changed. Note subclasses should
	 * override setValueRaw rather than this method so they do not need to
	 * handle the Observable methods.
	 * 
	 ...
	 * This is because setValue notifies any observers and then calls this
	 * method.
	 * 
	 * @param object
	 * @return false if tried to change a constant value.
	...
	/**
	 * Returns a string with the variable name followed by it's value. For
	 * example for the variable "a" with the value 10, the following string is
	 * returned:
	 * 
	 * <pre>
	 * a: 10
