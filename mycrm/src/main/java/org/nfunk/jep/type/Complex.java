 ...
 * components. Includes complex arithmetic functions.
 * <p>
 ...
 * 0-521-43108-5) http://www.nr.com and <br>
 * - The org.netlib.math.complex package (http://www.netlib.org) which was
 * developed by Sandy Anderson and Priyantha Jayanetti (published under GPL).
 * <p>
 ...
 * validated by comparing results with the netlib complex class.
 * <p>
 * It is important to note that the netlib complex package is more extensive and
 * efficient (e.g. Garbage collector friendly) than this implementation. If high
 * precision and efficiency if of necessity it is recommended to use the netlib
 * package.
 * 
 * @author Nathan Funk
 * @version 2.3.0 alpha now extends Number, has add and sub methods.
 * @version 2.3.0 beta 1 now overrides equals and hashCode. /* @version 2.3.0
 *          beta 2 does not implement Number anymore, as caused too many
 *          problems.
 */
...
public class Complex {
	...
	/**
	 * Constructor from a single double value. The complex number is initialized
	 * with the real component equal to the parameter, and the imaginary
	...
	/**
	 * Construct from a Number. This constructor uses the doubleValue() method
	 * of the parameter to initialize the real component of the complex number.
	...
	/**
	 * Initialize the real and imaginary components to the values given by the
	 * parameters.
	 */
	 ...
	 * and only if the argument is not null and is a Complex object that
	 * represents the same complex number. Equality follows the same pattern as
	 * Double aplies to each field:
	 * <ul>
	 * <li>If d1 and d2 both represent Double.NaN, then the equals method
	 * returns true, even though Double.NaN==Double.NaN has the value false.
	 * <li>If d1 represents +0.0 while d2 represents -0.0, or vice versa, the
	...
	public boolean equals(Object o) {
		if (!(o instanceof Complex))
			return false;
		Complex c = (Complex) o;
		return (Double.doubleToLongBits(this.re) == Double.doubleToLongBits(c.re) && Double.doubleToLongBits(this.im) == Double
				.doubleToLongBits(c.im));
	}
	...
	/**
	 * Prints using specified number format in format or "2" or "3 i" or
	 * "(2+3 i)" if flag is true or "2+3 i" if flag is false
	 */
	...
	/**
	 * Calculates the square root of this object. Adapted from Numerical Recipes
