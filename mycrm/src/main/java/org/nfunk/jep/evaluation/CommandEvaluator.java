...
public class CommandEvaluator {
	...
	public Object evaluate(CommandElement[] commands, SymbolTable symTab) throws Exception {
		...
		while (i < nCommands) {
			...
			/*
			 * if (command instanceof ASTVarNode) { // Variable
			 * stack.push(symTab.get(((ASTVarNode)command).getName())); } else
			 * if (command instanceof PostfixMathCommandI) { // Function pfmc =
			 * (PostfixMathCommandI)command;
			 * 
			 * // set the number of current parameters // (it is no faster to
			 * first check getNumberOfParameters()==-1) nParam =
			 * ((Integer)commands.elementAt(++i)).intValue();
