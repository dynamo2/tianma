...
-- Dump completed on 2012-05-06  0:02:55

/*
-- Inital values
insert into email_domain_white_list(`domain`) 
	select distinct substr(person.email,locate('@',person.email)) from person 
	where 
		email is not null and 
		trim(email) != '' and
		locate('@',person.email) > 0
		and not exists (
			select id from email_domain_white_list 
			where `domain` = substr(person.email,locate('@',person.email))
		)
*/